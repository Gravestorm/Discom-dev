...
const req = async (data, i) => {
  ...
  for (let j = 0; j < uris.length; j++) {
    await delay(6000)
    await fetch({ url: uris[j], headers: { Accept: '*/*', 'Accept-Language': 'en-US,en;q=0.5', Authorization: nconf.get('USER1'), Connection: 'close', 'Content-Type': 'application/json', 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:100.0) Gecko/20100101 Firefox/100.0' } }).then(res => {
...
module.exports = (client) => {
  const D = JSON.parse(fs.readFileSync('score.json'))
  for (let i = 0; i < D.length; i++) {
    console.log(D[i].N, D[i].S, D[i].A, D[i].S / D[i].A, D[i].A / D[i].S)
  }
  if (!nconf.get('USER122') || !nconf.get('SERVER') || !nconf.get('ROLE_IRON') || !nconf.get('ROLE_COPPER') || !nconf.get('ROLE_BRONZE') || !nconf.get('ROLE_SILVER') || !nconf.get('ROLE_GOLD') || !nconf.get('ROLE_CRYSTAL') || !nconf.get('ROLE_DIAMOND') || !nconf.get('ROLE_LEGEND') || !nconf.get('ROLE_EPIC') || !nconf.get('ROLE_OMEGA')) return
  ...
  client.guilds.fetch(nconf.get('SERVER')).then(g => g.members.fetch({ force: true }).then(m => m.forEach(m => {
    ...
    client.guilds.fetch(nconf.get('SERVER')).then(async g => {
      ...
      for (let i = 0; i < data.length; i++) { delete data[i].XX }
      fs.writeFileSync('data.json', JSON.stringify(data))
      console.log('+')
    })
  })
