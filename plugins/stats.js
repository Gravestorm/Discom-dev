...
module.exports = async (client) => {
  /*for (let i = 0; i < stats.length; i++) {
    stats[i].TotalEN = stats[i].GenEN + stats[i].MonoEN + stats[i].RetroEN + stats[i].TempoEN + stats[i].OffEN
    stats[i].TotalFR = stats[i].GenFR + stats[i].MonoFR + stats[i].RetroFR + stats[i].TempoFR + stats[i].OffFR
    delete stats[i].GenEN
    delete stats[i].MonoEN
    delete stats[i].RetroEN
    delete stats[i].TempoEN
    delete stats[i].OffEN
    delete stats[i].GenFR
    delete stats[i].MonoFR
    delete stats[i].RetroFR
    delete stats[i].TempoFR
    delete stats[i].OffFR
  }
  fs.writeFileSync('stats.json', JSON.stringify(stats))*/
  if (!nconf.get('USER999')) return
  ...
  for (let i = 0; i < dates.length; i++) {
    ...
    for (let j = 0; j < channels.length; j++) {
      await delay(6000)
      await fetch({ url: `https://discord.com/api/v9/guilds/78581046714572800/messages/search?channel_id=${channels[j]}&${dates[i]}`, headers: { Accept: '*/*', 'Accept-Language': 'en-US,en;q=0.5', Authorization: nconf.get('USER1'), Connection: 'close', 'Content-Type': 'application/json', 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:100.0) Gecko/20100101 Firefox/100.0' } }).then(res => {
    ...
    stats.push({ D: datename[i], GenEN: genen, MonoEN: monoen, RetroEN: retroen, TempoEN: tempoen, OffEN: offen, GenFR: genfr, MonoFR: monofr, RetroFR: retrofr, TempoFR: tempofr, OffFR: offfr, Other: other })
    fs.writeFileSync('stats.json', JSON.stringify(stats))
  }
}