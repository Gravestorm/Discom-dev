...
module.exports = (client) => {
  ...
  setInterval(() => {
    client.guilds.fetch(nconf.get('SERVER')).then(g => g.members.fetch().then(m => m.forEach(m => {
      if (m.displayName.startsWith('( ') || m.displayName.toLowerCase().startsWith('(agr) ') || m.displayName.toLowerCase().startsWith('(agride) ') || m.displayName.toLowerCase().startsWith('(ech) ') || m.displayName.toLowerCase().startsWith('(echo) ')) return m.setNickname(m.displayName.replace(/ /gi, ''))
